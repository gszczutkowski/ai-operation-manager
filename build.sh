...
TAG=$(git describe --tags --match "v*" --abbrev=0 2>/dev/null) || {
    echo "ERROR: No version tag found. Create one first: git tag v1.0.0" >&2
    exit 1
}
APP_VERSION="${TAG#v}"
echo "==> Version from git tag: $APP_VERSION"

VERSION_FILE="aom/_version.py"
VERSION_TUPLE=$(echo "$APP_VERSION" | tr '.' ', ')
cat > "$VERSION_FILE" <<VEOF
# file generated by build script from git tag
__version__ = version = '$APP_VERSION'
