...
# ---------------------------------------------------------------------------
# Extract version from the latest git tag (e.g. v1.0.2 -> 1.0.2)
# ---------------------------------------------------------------------------
$Tag = git describe --tags --match "v*" --abbrev=0 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Error "No version tag found. Create one first: git tag v1.0.0"
    exit 1
}
$AppVersion = $Tag -replace '^v', ''
Write-Host "==> Version from git tag: $AppVersion"

$VersionFile = "aom/_version.py"
Set-Content -Path $VersionFile -Value @"
# file generated by build script from git tag
__version__ = version = '$AppVersion'
version_tuple = ($($AppVersion -replace '\.', ', '))
"@
